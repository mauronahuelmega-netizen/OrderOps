...
export async function createBusinessAction(
  ...
  if (!name) {
    return { error: "Ingresá un nombre para el negocio." };
  }

  if (!slug) {
    return { error: "Ingresá un slug para el negocio." };
  }
...
export async function createClientAction(
  ...
  if (!email) {
    return { error: "Ingresá un email para el usuario admin." };
  }

  if (!password) {
    return { error: "Ingresá una contraseña para el usuario admin." };
  }
...
export async function createBusinessUserAction(
  ...
  if (!password) {
    return { error: "Ingresá una contraseña." };
  }

  if (!businessId) {
    return { error: "Seleccioná un negocio." };
  }

  if (!["owner", "manager", "operator", "viewer", "admin"].includes(role)) {
...
export async function updateBusinessUserAction(
  ...
  if (!["admin", "owner", "manager", "operator", "viewer", "super_admin"].includes(role)) {
    return { error: "Rol inválido." };
  }

  const nextRole = role as ProfileRole;

  if (nextRole !== "super_admin" && !businessId) {
    return { error: "Seleccioná un negocio para usuarios admin." };
  }

  const { error: profileError } = await serviceSupabase
    .from("profiles")
    .update({
      business_id: nextRole === "super_admin" ? null : businessId,
      role: nextRole
    })
    .eq("id", userId);
...
export async function deleteBusinessUserAction(
  ...
  if (!confirmEmail) {
    return { error: "Escribí el email para confirmar." };
  }
  ...
  if (!targetEmail || confirmEmail !== targetEmail) {
    return { error: "La confirmación no coincide con el email del usuario." };
  }
  ...
  if (targetProfile.role === "super_admin") {
    ...
    if ((count ?? 0) <= 1) {
      return { error: "No podés eliminar el último super admin." };
    }
  }
...
export async function deleteBusinessAction(
  ...
  if (!confirmSlug) {
    return { error: "Escribí el slug para confirmar." };
  }
  ...
  if (confirmSlug !== business.slug) {
    return { error: "La confirmación no coincide con el slug del negocio." };
  }

  const businessProfiles = profiles ?? [];

  if (businessProfiles.some((profile) => profile.id === context.user.id)) {
    return { error: "No podés eliminar un negocio que contiene tu propio usuario." };
  }
  ...
  if ((superAdminCount ?? 0) - superAdminsInBusiness < 1) {
    return { error: "No podés eliminar el último super admin del sistema." };
  }
  ...
  if (categoriesDeleteError) {
    return { error: categoriesDeleteError.message || "No pudimos eliminar las categorías." };
  }
