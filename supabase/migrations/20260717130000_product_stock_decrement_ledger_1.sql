-- PRODUCT-STOCK-DECREMENT-LEDGER-1
-- Record order_decrement rows in stock_movements when create_order decrements track_stock=true.
-- Preserves signature and PRODUCT-STOCK-DECREMENT-ORDER-1 behavior.
-- Does NOT implement restock. Does NOT alter stock_movements schema. Does NOT backfill.
...
comment on function public.create_order(
  ...
  'STOCK-DECREMENT-LEDGER-1: creates orders + order_items; validates/decrements track_stock=true; records stock_movements.order_decrement per tracked order_item. Legacy track_stock=false unchanged. No cancel restock.';
