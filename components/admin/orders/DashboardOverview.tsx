...
function KpiGrid({ title, items, variant }: KpiGridProps) {
  return (
    <section className={styles.kpiSection} data-section={variant} aria-label={title}>
      ...
      <div className={`${styles.kpiGrid} ${styles[`kpiGrid--${variant}`]}`}>
        {items.map((item) => (
          <article
            key={item.id}
...
export default function DashboardOverview({ viewModel }: DashboardOverviewProps) {
  return (
    <section className={styles.root} aria-label="Resumen operativo del dashboard">
